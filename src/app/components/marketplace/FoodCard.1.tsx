import React from 'react';
import Image from 'next/image';
import LogoBlue from '@/assets/logo-blue.png';
import { FoodCardProps } from './FoodCard';

export const FoodCard: React.FC<FoodCardProps> = ({
  imageUrl,
  chefName,
  description,
  deliveryTime,
  rating,
}) => {
  return (
    <div className="shadow-lg min-w-[150px] md:min-w-[290px] md:min-h-[230px] md:max-w-[470px] lg:min-w-[310px] lg:max-w-[500px] rounded-xl overflow-hidden flex flex-col gap-4 relative z-0 ">
      {/* Discount tag */}
      {/* Discount tag */}
      <div className="text-xs lg:flex lg:items-center text-white gap-1 absolute z-10 px-2 py-1 hidden top-4 left-4 bg-nana-blue rounded-lg max-w-full min-w-fit">
        <svg
          fill="#fff"
          height="14px"
          width="14px"
          version="1.1"
          id="Layer_1"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 511.999 511.999"
        >
          <g>
            <g>
              <path
                d="M196.88,125.972c-6.527-6.527-17.113-6.529-23.641,0l-47.284,47.283c-6.529,6.527-6.529,17.113,0,23.641
			c6.527,6.529,17.113,6.529,23.641,0l47.284-47.283C203.409,143.086,203.409,132.5,196.88,125.972z"
              />
            </g>
          </g>
          <g>
            <g>
              <path
                d="M492.398,326.925c12.63-12.63,19.586-29.422,19.586-47.284c0-17.861-6.955-34.653-19.586-47.283L279.625,19.585
			C266.995,6.955,250.203,0,232.342,0s-34.653,6.955-47.283,19.585l-12.806,12.806C133.112,4.535,78.292,8.147,43.211,43.227
			C7.459,78.978,4.917,133.846,32.365,172.28l-12.795,12.795c-26.072,26.072-26.072,68.493,0,94.565l212.774,212.774
			c12.63,12.63,29.422,19.585,47.284,19.585s34.653-6.955,47.283-19.585l12.795-12.795c38.444,27.454,93.315,24.892,129.053-10.846
			c35.738-35.738,38.302-90.609,10.846-129.053L492.398,326.925z M468.757,303.283l-23.641,23.641
			c-6.524,6.523-6.529,17.113,0,23.641c26.132,26.132,26.137,68.43,0,94.566c-26.132,26.131-68.43,26.137-94.566,0
			c-6.526-6.529-17.112-6.527-23.641,0l-23.642,23.641c-13.066,13.066-34.214,13.067-47.284,0L66.851,279.642l35.461-35.461
			c6.529-6.529,6.529-17.113,0-23.641c-6.527-6.529-17.113-6.529-23.641,0l-35.461,35.461c-13.065-13.066-13.068-34.214,0-47.283
			l23.642-23.641c6.524-6.523,6.529-17.114,0-23.641c-12.63-12.63-19.586-29.422-19.586-47.284
			c0-36.955,29.908-66.868,66.868-66.868c17.861,0,34.653,6.957,47.284,19.586c6.527,6.529,17.113,6.527,23.641,0l23.642-23.641
			c6.315-6.315,14.711-9.793,23.641-9.793c8.931,0,17.327,3.478,23.642,9.793l-35.462,35.462c-6.529,6.527-6.529,17.113,0,23.641
			c6.527,6.529,17.112,6.529,23.641,0l35.461-35.464l189.131,189.132C481.825,269.069,481.824,290.215,468.757,303.283z"
              />
            </g>
          </g>
          <g>
            <g>
              <path
                d="M279.625,145.905c-9.232,0-16.717,7.485-16.717,16.717v234.039c0,9.232,7.485,16.717,16.717,16.717
			c9.232,0,16.717-7.485,16.717-16.717V162.622C296.342,153.39,288.858,145.905,279.625,145.905z"
              />
            </g>
          </g>
          <g>
            <g>
              <path
                d="M179.323,229.491c-27.653,0-50.151,22.498-50.151,50.151c0,27.653,22.498,50.151,50.151,50.151
			c27.653,0,50.151-22.498,50.151-50.151C229.474,251.988,206.976,229.491,179.323,229.491z M179.323,296.359
			c-9.218,0-16.717-7.499-16.717-16.717s7.499-16.717,16.717-16.717s16.717,7.499,16.717,16.717S188.541,296.359,179.323,296.359z"
              />
            </g>
          </g>
          <g>
            <g>
              <path
                d="M379.928,229.491c-27.653,0-50.151,22.498-50.151,50.151c0,27.653,22.498,50.151,50.151,50.151
			c27.653,0,50.151-22.498,50.151-50.151C430.079,251.988,407.581,229.491,379.928,229.491z M379.928,296.359
			c-9.218,0-16.717-7.499-16.717-16.717s7.499-16.717,16.717-16.717c9.218,0,16.717,7.499,16.717,16.717
			S389.146,296.359,379.928,296.359z"
              />
            </g>
          </g>
        </svg>
        <span>₦0.00 for 14days delivery</span>
      </div>
      {/* Card content */}
      <div className="w-full">
        <Image
          src={imageUrl}
          alt="Test"
          width={0}
          height={0}
          className="w-full object-cover"
        />
      </div>
      <div className="w-full">
        <div className="mb-6 px-4 flex justify-between gap-2">
          <div className="space-y-2 truncate">
            <h5 className="font-semibold text-lg lg:text-base">{chefName}</h5>
            <p className="text-gray-500 text-base lg:text-sm lg:hidden">
              Pineapple, burger
            </p>
            <p className="text-gray-500 lg:text-sm hidden text-ellipsis lg:block overflow-hidden">
              {description}
            </p>
          </div>
          <div className="lg:flex-col text-sm bg-nana-blue/10  rounded-lg px-2 py-1 min-h-10 min-w-14 justify-center items-center text-nana-blue hidden lg:flex">
            <div>{deliveryTime}</div>
            <div>min</div>
          </div>
        </div>
        <div className="border-dashed border-t-2 px-4 py-2">
          <div className="flex items-center">
            <span className="flex gap-1 items-center justify-center text-xs">
              <Image
                src={LogoBlue}
                alt="nana logo"
                width={0}
                height={0}
                className="size-4"
              />
              <span className="lg:text-xs text-nana-blue">₦0.00</span>
            </span>
            <span className="text-xs hidden before:content-['·'] before:relative before:inline-block before:my-0 before:mx-1 lg:flex gap-1 items-center">
              <span>
                ₦<span className="text-gray-500 ">₦₦₦</span>
              </span>
            </span>
            <span className="text-xs hidden before:content-['·'] before:relative before:inline-block before:my-0 before:mx-1 lg:flex gap-1 items-center">
              <svg
                width="12px"
                height="12px"
                viewBox="0 -2.5 160 160"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M77.641 109.484C72.9009 109.484 65.9845 107.282 59.4934 103.692C55.9045 101.703 52.8079 99.087 50.9573 95.2983C50.3885 94.3292 50.0498 93.2424 49.9673 92.1217C49.9971 91.0383 50.0351 90.0267 51.0894 89.5737C51.6234 89.3877 52.1991 89.3564 52.7502 89.4832C53.3013 89.6101 53.8053 89.89 54.2042 90.2908C55.4807 91.5232 56.1747 93.4206 57.5453 94.4817C66.8574 101.695 77.1602 104.852 88.8085 101.248C94.2358 99.5683 98.0015 95.7554 100.849 90.9551C101.535 89.7978 102.028 88.3284 103.041 87.614C104.176 86.8143 106.105 86.0328 107.127 86.492C108.149 86.9511 108.975 88.9466 108.99 90.2786C109.005 91.7256 108.188 93.3061 107.389 94.619C101.248 104.714 92.1908 109.602 77.641 109.484Z"
                  fill="#000000"
                />
                <path
                  d="M48.7689 46.7527C56.5141 46.5808 61.2223 49.154 65.1558 53.3597C65.8554 54.0913 66.4025 54.9547 66.7654 55.8997C67.6822 58.4559 65.8553 60.6607 63.2585 59.8447C61.3835 59.1397 59.6673 58.0693 58.2096 56.6954C54.6627 53.6259 50.8044 52.8098 46.2844 54.0463C39.6382 55.8644 34.3666 59.2137 31.658 65.8659C31.0412 67.38 30.0464 68.7838 28.0956 68.1371C26.0134 67.4471 25.7736 65.4705 26.4596 63.9388C27.8179 60.9072 29.2135 57.709 31.3655 55.253C36.3662 49.5493 43.2061 47.3906 48.7689 46.7527Z"
                  fill="#000000"
                />
                <path
                  d="M128.999 55.286C128.647 55.7519 128.071 57.2802 127.103 57.5931C126.476 57.7121 125.831 57.6926 125.212 57.5359C124.594 57.3791 124.017 57.089 123.523 56.6858C122.394 55.6958 121.753 54.1234 120.986 52.7542C120.513 51.7836 119.747 50.9866 118.796 50.4761C117.844 49.9656 116.756 49.7676 115.686 49.9102C109.777 50.4194 104.475 52.115 100.66 57.1178C100.081 57.8775 98.0438 57.8937 97.6625 57.5449C96.8499 56.8001 96.0956 55.8521 96.382 54.9129C96.9428 53.2144 97.9063 51.677 99.1901 50.4316C104.299 45.4302 110.651 43.4299 117.672 43.6236C122.98 43.7671 128.8 49.51 128.999 55.286Z"
                  fill="#000000"
                />
                <path
                  d="M44.2051 75.5854C45.3123 76.1089 46.525 77.6676 46.5751 78.8113C46.5179 79.5021 46.3007 80.1701 45.9409 80.7626C45.581 81.355 45.0884 81.8556 44.5017 82.2248C40.3135 84.37 35.7929 85.2273 31.1368 83.9441C29.4047 83.4701 27.7274 82.5953 27.5967 80.3804C27.4701 78.2352 28.8027 76.9533 30.5375 76.3114C33.0281 75.3904 42.4391 74.7505 44.2051 75.5854Z"
                  fill="#000000"
                />
                <path
                  d="M120.918 78.5489C117.829 77.3348 117.058 74.0643 119.461 71.8148C120.841 70.5528 122.524 69.6701 124.347 69.253C126.265 68.8379 128.375 69.0864 130.374 69.3099C132.321 69.5279 134.129 70.2457 134.66 72.4776C134.962 73.4622 134.91 74.5213 134.513 75.4717C134.116 76.422 133.4 77.2037 132.488 77.6815C130.481 78.7595 122.457 79.1543 120.918 78.5489Z"
                  fill="#000000"
                />
                <path
                  d="M3.27272 55.9748C-3.73711 83.5259 2.40121 107.831 21.5299 128.223C22.0283 128.75 22.5355 129.277 23.0434 129.785C33.3184 140.06 46.1171 146.404 56.2588 150.819C60.6115 152.719 82.9451 155.028 88.988 154.128C124.899 148.765 148.531 128.511 156.762 94.625C158.333 87.9873 159.116 81.1873 159.094 74.3661C159.132 58.2974 152.366 39.6163 138.414 25.6251C136.977 24.4373 135.546 23.2069 134.162 22.0145C131.16 19.4339 128.057 16.762 124.81 14.4834C100.013 -2.97008 74.2479 -3.06774 48.2339 6.1875C24.6807 14.5618 9.55529 31.3158 3.27272 55.9748ZM48.3341 14.4373C71.1594 4.95246 94.0612 3.27855 116.398 17.4844C120.482 20.0847 131.371 28.8849 133.085 30.2067L133.195 30.2879L133.286 30.3889C142 39.6299 151.978 56.5268 151.758 73.8963C151.686 81.3232 150.68 88.7115 148.766 95.8878C143.092 116.511 129.527 131.541 107.295 141.821C84.6956 152.642 52.2846 141.702 43.6611 136.01C34.1295 129.493 25.7895 121.386 19.0056 112.043C8.36622 96.9802 5.97929 75.3276 11.6958 55.5339C14.3715 46.3948 19.0474 37.9656 25.3844 30.8574C31.7214 23.7493 39.5608 18.1403 48.3341 14.4373Z"
                  fill="#000000"
                />
              </svg>
              <span className="text-gray-500 ">{rating}</span>
            </span>
            <span className="text-xs lg:hidden before:content-['·'] before:relative before:inline-block before:my-0 before:mx-1 flex gap-1 items-center">
              <span className="text-gray-500 ">{deliveryTime} min</span>
            </span>
          </div>
        </div>
      </div>
    </div>
  );
};
